...
- **numeric vector of length 4** in decimal degrees makes a square map
...
```{r eval = FALSE}
basemap(shapefiles = list(land = bs_land, glacier = bs_glacier, bathy = bs_bathy), 
...
lims <- auto_limits(expand.grid(data.frame(lon = bound.limits[1:2], lat = bound.limits[3:4])), proj.out = projection)$projBound
sp::plot(lims)
...
p + geom_path(data = cur, 
              aes(x = long, y = lat, group = group, color = type, size = size),
...
# And thanks to you!
[.black[mikkovihtakari.github.io/ggOceanMaps/]](https://mikkovihtakari.github.io/ggOceanMaps/) <br>
[.small[.black[The presentation source code]]](https://github.com/AeN-R-Workshop/4-ggOceanMaps)
